 ...
 * concrete sizing before that hardening exists. Exposed as `val` (not
 * `private val`) so Phase 7's [PositionCaps.clamp] can convert its own
 * notional ceiling into an equivalent fraction ceiling using the exact
 * same scale this emitter will use to size the resulting order - one
 * source of truth for base-coin-per-unit-fraction, not two that could
 * quietly disagree.
 *
...
class PositionOrderEmitter(
    ...
    val maxPositionSizeBaseCoin: Double,
