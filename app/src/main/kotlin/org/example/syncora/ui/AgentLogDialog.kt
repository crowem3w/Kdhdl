...
/**
 * Shows only the AGENT-source entries from [AppLog], streamed live the same way
 * as [LogPanelDialog], but pre-filtered so this panel is dedicated to agent activity.
 */
class AgentLogDialog(context: Context) : Dialog(context, R.style.TradingModalTheme) {
    ...
    override fun onStart() {
        ...
        collectJob = dialogScope.launch {
            AppLog.entries.collect { entries ->
                renderEntries(entries.filter { it.source == LogSource.AGENT })
            }
        }
    }
    ...
    private fun buildTerminalContent(): View {
        ...
        emptyStateText = TextView(context).apply {
            text = "No agent activity yet. Agent events will stream here."
            typeface = Typeface.MONOSPACE
            textSize = 11.5f
    ...
    private fun appendLine(builder: SpannableStringBuilder, entry: LogEntry) {
        ...
        builder.setSpan(ForegroundColorSpan(SOURCE_AGENT), tagStart, builder.length, 0)
