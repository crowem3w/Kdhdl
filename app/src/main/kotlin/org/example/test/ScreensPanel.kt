...
private fun buildScreenThumbnailTile(context: Context, page: ScreenPage, selected: Boolean, onClick: () -> Unit): View {
    ...
    tile.addView(TextView(context).apply {
        // Always the screen TYPE (Home/Onboarding/Splash/Blank), not the page's custom display
        // name - that name is only shown/edited via tvScreenName in the top bar (see
        // startScreenNameEdit() in SketchActivity), independent of this thumbnail label.
