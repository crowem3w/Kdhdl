...
enum class SocketState { IDLE, CONNECTING, CONNECTED, RECONNECTING, FAILED, STOPPED }

class BitgetKlineSocket(
    private val instId: String = "BTCUSDT",
    private val instType: String = "USDT-FUTURES",
    initialGranularity: String = "candle1m",
) {
    private companion object {
        const val TAG = "BitgetKlineSocket"
    ...
    private var granularity: String = initialGranularity
    ...
    fun switchGranularity(newGranularity: String) {
        ...
        if (ws != null && _state.value == SocketState.CONNECTED) {
            unsubscribe(ws, oldGranularity)
            subscribe(ws, newGranularity)
        }
    }
    ...
    private fun subscribe(ws: WebSocket, channel: String = granularity) {
        ws.send(subscriptionPayload("subscribe", channel))
    }

    private fun unsubscribe(ws: WebSocket, channel: String) {
        ws.send(subscriptionPayload("unsubscribe", channel))
    }

    private fun subscriptionPayload(op: String, channel: String): String =
        JSONObject().apply {
            put("op", op)
            put(
                "args",
    ...
    private fun handleMessage(text: String) {
        try {
            ...
            val updates = buildList(rows.length()) {
