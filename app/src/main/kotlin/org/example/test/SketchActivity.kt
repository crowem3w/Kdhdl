...
class SketchActivity : AppCompatActivity() {
    ...
    // Tappable current-screen name shown next to btnBack in topBar, and the inline EditText that
    // swaps into its place while renaming (same pattern as nameTagEditor/editingNamePart above,
    // but simpler: topBar doesn't pan/zoom, so screenNameEditor sits in a fixed layout slot next
    // to tvScreenName instead of being positioned over dynamic canvas coordinates). See
    // startScreenNameEdit()/commitScreenNameEdit().
    private lateinit var tvScreenName: TextView
    private lateinit var screenNameEditor: EditText
    ...
    private fun setupTopBar() {
        ...
        screenNameEditor = findViewById(R.id.screenNameEditor)
        setupScreenNameEditor()
        tvScreenName.text = currentScreenPage()?.name.orEmpty()
        tvScreenName.setOnClickListener { startScreenNameEdit() }
    ...
    private fun addScreenPage(type: ScreenPageType) {
        ...
        tvScreenName.text = page.name
    }

    
    private fun selectScreenPage(page: ScreenPage) {
        if (screenNameEditor.visibility == View.VISIBLE) commitScreenNameEdit()
    ...
    override fun dispatchTouchEvent(ev: android.view.MotionEvent): Boolean {
        ...
        if (screenNameEditor.visibility == View.VISIBLE && ev.actionMasked == android.view.MotionEvent.ACTION_DOWN) {
            val rect = android.graphics.Rect()
            screenNameEditor.getGlobalVisibleRect(rect)
            if (!rect.contains(ev.rawX.roundToInt(), ev.rawY.roundToInt())) {
                commitScreenNameEdit()
            }
        }
        return super.dispatchTouchEvent(ev)
    }

    // Returns the ScreenPage currently shown on canvas (by selectedScreenPageId), or null in the
    // (should-never-happen) case the id doesn't match any entry.
    private fun currentScreenPage(): ScreenPage? = screenPages.find { it.id == selectedScreenPageId }

    private fun setupScreenNameEditor() {
        screenNameEditor.setOnEditorActionListener { _, actionId, _ ->
            if (actionId == EditorInfo.IME_ACTION_DONE) {
                commitScreenNameEdit()
                true
            } else {
        ...
        screenNameEditor.setOnFocusChangeListener { _, hasFocus ->
            if (!hasFocus) commitScreenNameEdit()
        }
    }

    // Swaps tvScreenName out for screenNameEditor (same slot in topBar) pre-filled with the
    // active page's current name, ready to type into.
    private fun startScreenNameEdit() {
        val page = currentScreenPage() ?: return
        tvScreenName.visibility = View.GONE
        screenNameEditor.visibility = View.VISIBLE
        screenNameEditor.setText(page.name)
        screenNameEditor.requestFocus()
        screenNameEditor.setSelection(screenNameEditor.text?.length ?: 0)
        val imm = getSystemService(INPUT_METHOD_SERVICE) as? InputMethodManager
        imm?.showSoftInput(screenNameEditor, InputMethodManager.SHOW_IMPLICIT)
    }

    // Commits the typed text as the active ScreenPage's display name (falling back to its type
    // label - Home/Onboarding/Splash/Blank - when left empty), persists it by replacing that page
    // in screenPages (ScreenPage.name is a val, so renaming means swapping in a .copy()), and
    // swaps screenNameEditor back out for tvScreenName. Does NOT touch the page-thumbnails' own
    // label, which always shows the type instead - see buildScreenThumbnailTile() in
    // ScreensPanel.kt.
    private fun commitScreenNameEdit() {
        if (screenNameEditor.visibility != View.VISIBLE) return
        val page = currentScreenPage()
        if (page != null) {
            val typed = screenNameEditor.text?.toString()?.trim().orEmpty()
            val newName = typed.ifEmpty { page.type.label }
            val index = screenPages.indexOfFirst { it.id == page.id }
            if (index != -1) screenPages[index] = page.copy(name = newName)
            tvScreenName.text = newName
        }
        screenNameEditor.visibility = View.GONE
        tvScreenName.visibility = View.VISIBLE
        val imm = getSystemService(INPUT_METHOD_SERVICE) as? InputMethodManager
        imm?.hideSoftInputFromWindow(screenNameEditor.windowToken, 0)
    }
