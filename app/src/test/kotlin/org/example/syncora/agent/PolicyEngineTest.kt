 ...
 * Prompt 6). The central test here is [`the RTRL trace matches a finite-difference gradient over a multi-step recurrent rollout`]:
...
class PolicyEngineTest {
    ...
    fun `output is always within the bounded position range regardless of extreme inputs`() {
        ...
        repeat(20) {
            val f = engine.step(extremeState, readoutForecast = 500f)
    ...
    fun `two engines with identical seed and identical input sequence produce identical trajectories`() {
        ...
        repeat(50) {
            ...
            val forecast = rng.nextFloat() * 2f - 1f
            val fa = a.step(state, forecast)
            val fb = b.step(state, forecast)
    ...
    fun `single-step trace matches a finite-difference gradient with no recurrent feedback`() {
        ...
        for (i in baseWeights.indices) {
            ...
            val fPerturbed = perturbedEngine.step(state, forecast)

            val finiteDiffGradient = (fPerturbed - f) / epsilon
            val analyticGradient = analyticTrace(i, f, state, forecast, nBack)
            assertEquals(
    ...
    private fun analyticTrace(
        ...
        val regressor = FloatArray(state.size + 1 + nBack + 1)
        System.arraycopy(state, 0, regressor, 0, state.size)
        regressor[state.size] = forecast
    ...
    fun `the RTRL trace matches a finite-difference gradient over a multi-step recurrent rollout`() {
        val nBack = 3
        val steps = 6
        val nRegressors = nHidden + 1 + nBack + 1
        ...
        val forecasts = List(steps) { stateRng.nextFloat() * 0.5f - 0.25f }
        ...
        // ~14 weights keeps the test fast while still covering every regressor
        // category (reservoir, readout, feedback, bias).
        val feedbackBase = nHidden + 1
        val indicesToCheck = listOf(0, 1, nHidden, feedbackBase, feedbackBase + 1, feedbackBase + 2, nRegressors - 1)

        val baseEngine = PolicyEngine(nHidden = nHidden, nBack = nBack, initialWeights = baseWeights.copyOf())
        var baseFinal = 0f
        for (t in 0 until steps) baseFinal = baseEngine.step(states[t], forecasts[t])
        ...
        for (i in indicesToCheck) {
            ...
            for (t in 0 until steps) perturbedFinal = perturbedEngine.step(states[t], forecasts[t])
            ...
            for (t in 0 until steps) probeEngine.step(states[t], forecasts[t])
    ...
    fun `stays stable across a long replay with continuous updates`() {
        ...
        repeat(5000) {
            val state = randomState(rng)
            val forecast = rng.nextFloat() * 0.1f - 0.05f
            engine.step(state, forecast)
    ...
    fun `resetState clears recurrent history but preserves trained weights`() {
        ...
        val rng = Random(8L)
        repeat(10) {
            engine.step(randomState(rng), rng.nextFloat())
